			...
			result.forEach(function(line_win) {
				if(!nohu && line_win.win == 10) {
					...
					if (line_win.type === 5) {
						...
						if(okHu>45000000)
						io.sendInHome({pushnohu:{title:'CowBoy', name:user.name, bet:okHu}});
						nohu = true;
						Zeus_red.create({'name':user.name, 'type':2, 'win':bet_win, 'bet':bet, 'kq':1, 'line':1, 'time':new Date()});
 			...
 			if (!nohu && bet_win >= tongCuoc*2.24) {
				io.sendInHome({news:{t:{game:'CowBoy', users:user.name, bet:bet_win, status:2}}});
			}
			HU.updateOne({game:'Zeus', type:bet}, {$inc:{bet:addQuy}}).exec();

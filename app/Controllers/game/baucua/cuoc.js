		...
		if (cuoc < 1000 || linhVat < 0 || linhVat > 5) {
			...
			UserInfo.findOne({id: client.UID}, 'red', function(err, user){
				if (!user || user.red < cuoc) {
					...
					if(client.timeCacheBauCua == undefined)
						client.timeCacheBauCua = now - 250
					
					
					if(now - client.timeCacheBauCua < 250)
						return client.red({baucua:{notice: 'Bạn đặt quá nhanh'}});
					client.timeCacheBauCua = now
					...
					BauCua_cuoc.findOne({uid:client.UID, phien:client.redT.BauCua_phien}, function(err, checkOne) {
					    ...
					    Push.create({
							type:"GameBauCuaBet",
							data:JSON.stringify({name:client.profile.name,uid:client.UID,money:cuoc,box:linhVat})
						});
					});
